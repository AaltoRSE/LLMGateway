...
class MongoUserRepository(UserRepository):
    ...
    def _convert_model_to_schema(self, db_user: DBUser) -> User:
        return User(
            ...
            admin=db_user.admin,
            store_data=db_user.store_data,
            accepted_agreement_version=db_user.accepted_agreement_version,
            created_at=db_user.created_at,
            last_active=db_user.last_active,
            seen_tiptour=db_user.seen_tiptour,
            selected_language=db_user.selected_language,
        )
    ...
    async def create_new_user(self, user: UserBase) -> User:
        db_user = DBUser(
            ...
            seen_tiptour=user.seen_tiptour,
    ...
    async def update_user(self, user: User) -> User | None:
        ...
        db_user.store_data = user.store_data
        db_user.accepted_agreement_version = user.accepted_agreement_version
        db_user.last_active = user.last_active
        db_user.seen_tiptour = user.seen_tiptour
        db_user.selected_language = user.selected_language
        self.db.commit()
        self.db.refresh(db_user)
        return self._convert_model_to_schema(db_user)
