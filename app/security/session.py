...
class SessionHandler:
    ...
    def create_session(
        ...
        logging.debug("Creating session with following data:")
        logging.debug(dataToSet)
        logging.debug("Obtained result from db")
        ...
        logging.debug("Returning key : " + session_key)
        return session_key
    ...
    def get_session_data(self, session_key):
        ...
        logging.debug(entry)
        logging.debug(f"Current time:{ctime} // Session Expired at: {entry["expire"]}")
        if ctime >= entry["expire"]:
            # This is expired, we can remove it... 
            self.session_collection.delete_one({"key": session_key})
            return None
        logging.debug("Session valid, returning")
        return entry["data"]
