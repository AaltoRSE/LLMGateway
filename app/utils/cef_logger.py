...
def format_cef_message(
    ...
    cef_params = OrderedDict([
        ...
        ("suser", authenticated_user.user.auth_id if authenticated_user else 'unknown'),
        ...
        # Source IP
        ("src", request.scope['client'][0] if request.scope['client'] else 'unknown'),
        ...
        # Referer, if any
        ("requestContext", request.headers.get('origin', 'unknown')), # by spec refererer, using origin here as this is an API endpoint
        ...
        # The request URL
        ("request", f"{request.scope['scheme']}://{request.headers.get('host', 'unknown')}{request.url.path}{request.url.query}")
    ])
