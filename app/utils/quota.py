...
endpoints = {
    'gpt4' : {"endpoint" : "https://...", "prompt" : 0.01, "completion" : 0.028},
    'gpt3' : {"endpoint" : "https://...", "prompt" : 0.0005, "completion" : 0.0014},
    'gpt3_instruct' : {"endpoint" : "https://...", "prompt" : 0.0014, "completion" : 0.002},    
}
...
class Quota:
    ...
    def get_endpoint(self):
        ...
        if current_price < 10:
            ...
            return endpoints["gpt3_instruct"]["endpoint"]
        elif current_price < 30:
            return endpoints["gpt3"]["endpoint"]
        else:
            raise HTTPException(status_code=status.HTTP_410_GONE, detail="Quota exceeded")
    def update_price(self, token_number : int, is_prompt : bool):
        ...
        """
        self.add_price(self.get_price())
    
    def get_price(self, token_number : int, is_prompt : bool):
        """ This function retrieves the price of the prompt based on the current endpoint.
            the number of tokens is used and the type of token (whether prompt or completion) 
            is used to calculate the price. 
        """
        endpoint = self.get_endpoint()
        cost = endpoints[endpoint]["prompt"] if is_prompt else endpoints[endpoint]["completion"] 
